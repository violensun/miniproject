...
passport.use(
    new LocalStrategy({
        ...
        if (index !== db.NOT_FOUND && await db.isValidUser(username, password)) {
            const { id, username, email,phone,facebook,line } = users.users[index]
            return cb(null,
                { id, username, email ,phone,facebook,line},
        ...
        (jwtPayload, cb) => {
            try {
                ...
                if (index !== db.NOT_FOUND) {
                    ...
                    return cb(null, { id, username, email ,phone,facebook,line}); 
                } else {
